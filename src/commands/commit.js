...
const commitOperations = [
  { name: `✅ ADD ${chalk.gray("- Adding something that was previously missing")}`, value: "[ADD]" },
...
async function commit() {
  ...
  const unstagedChanges = await execGit(
      ["diff", "--name-only"],
      ADDONS_PATH
  );
  const stagedChanges = await execGit(
  ...
  if (!stagedChanges.trim()) {
    ...
    stageFilesAnswers = await inquirer.prompt([{
      ...
      type: "checkbox",
      name: "filesToStage",
      choices: unstagedChanges.trim().split("\n").map((file) => (file))
    }]);
  } else {
