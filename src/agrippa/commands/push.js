...
import { pushPbEntry, publish } from './pushPb.js';
...
async function push(opts = {}) {
    ...
    if (stale.length) {
        ...
        const { cleanup } = await inquirer.prompt([
            { type: 'confirm', name: 'cleanup', message: 'Remove these entries from the workspace config?', default: true },
        ]);
        if (cleanup) {
    ...
    const hasCode = config.workspace.some((e) => e.object_type === 'phase' || e.object_type === 'mfa');
    const hasPb = config.workspace.some((e) => e.object_type === 'process_builder');
    const ripUrl = process.env.RIP_URL;
    if (hasCode && !ripUrl) throw new Error('RIP_URL is not configured. Run `prbot init` or set it in agrippa.yaml.');
    if (hasPb && !process.env.PB_URL) throw new Error('PB_URL is not configured. Run `prbot init` or set it in agrippa.yaml.');

    console.log('Fetching remote state...');
    const token = await getToken();

    const remoteCodeMap = hasCode ? await fetchRemoteCode(token, ripUrl, config.workspace) : new Map();
    ...
    const classified = config.workspace.map((entry) => {
        ...
        let status;
        if (pullChecksum === localChecksumVal && pullChecksum === remoteChecksum) status = 'unchanged';
    ...
    for (const entry of selected) {
        const idx = config.workspace.findIndex(
            (e) =>
                ...
                (entry.object_type === 'process_builder' ? e.guid === entry.guid : e.id === entry.id),
        );

        if (entry.object_type === 'process_builder') {
            const res = await pushPbEntry(token, entry, BACKUP_DIR, backupTs);
            const note = [res.created && `${res.created} new page(s)`, res.updated && `${res.updated} page(s) updated`]
                .filter(Boolean)
                .join(', ');
    ...
    for (const { entry, idx } of pushedPb) {
        ...
        if (opts.skipPublish) doPublish = false;
            ...
            ({ doPublish } = await inquirer.prompt([
                { type: 'confirm', name: 'doPublish', message: `Publish "${entry.name}" now?`, default: false },
            ]));
        }
        if (doPublish) {
